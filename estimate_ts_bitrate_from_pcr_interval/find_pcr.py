...
with sys.stdin as f:
    for line in f:
        ...
        if m and PID == pcr_pid:
            ...
            m_pcr = re.search("PCR-Timestamp: ([0-9]{1,2}):([0-9]{2}):([0-9]{2}).([0-9]{6})", line)
            if m_pcr:
                m_pcr = m_pcr.groups()
                ...
                if prev_pcr:
                    bitrate = (((current_pcr[0] - prev_pcr[0]) * 8 * 188) / ((current_pcr[1] - prev_pcr[1]) / 1000 )) / (1000*1000)
                    print 'bitrate = %f Mbps, packets_diff = %d, pcr_diff = %f pcr = %f' %\
                        (bitrate, (current_pcr[0] - prev_pcr[0]), (current_pcr[1] - prev_pcr[1]), current_pcr[1]/1000)
